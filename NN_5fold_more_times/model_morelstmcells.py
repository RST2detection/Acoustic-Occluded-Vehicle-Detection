...
class C_lenet_morelstmcells(nn.Module):
    def __init__(self, nn_number, class_number, lstm_layer):
        super(C_lenet_morelstmcells, self).__init__()
        ...
        self.lstm_parallel_3 = nn.LSTM(input_size=61,
                                       ...
                                       batch_first=True)
        # 如果还不满意，可以改4*4这个参数
        self.fc1_pCRNN = nn.Linear(2 * 32 * 4 * 4 + 2 * 256, 2 * 1024)
        self.fc1_pCNN = nn.Linear(2 * 32 * 4 * 4, 2 * 1024)
        self.fc1_CRNN = nn.Linear(32 * 4 * 4 + 256, 2 * 512)
